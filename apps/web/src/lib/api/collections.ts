...
export async function getCollections(signal?: AbortSignal): Promise<Collection[]> {
  return parseCollectionList(await fetch('/api/collections', signal === undefined ? undefined : { signal }))
}
...
export async function updateCollection(id: string, name: string): Promise<Collection> {
  const response = await fetch(`/api/collections/${encodeURIComponent(id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) })
...
export async function deleteCollection(id: string): Promise<void> {
  const response = await fetch(`/api/collections/${encodeURIComponent(id)}`, { method: 'DELETE' })
...
export async function removeMediaFromCollection(collectionId: string, mediaId: string): Promise<void> {
  const response = await fetch(`/api/collections/${encodeURIComponent(collectionId)}/media/${encodeURIComponent(mediaId)}`, { method: 'DELETE' })
...
export async function getMediaCollections(mediaId: string, signal?: AbortSignal): Promise<Collection[]> {
  return parseCollectionList(await fetch(`/api/media/${encodeURIComponent(mediaId)}/collections`, signal === undefined ? undefined : { signal }))
}

export async function setMediaCollections(mediaId: string, collectionIds: string[]): Promise<Collection[]> {
  return parseCollectionList(await fetch(`/api/media/${encodeURIComponent(mediaId)}/collections`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ collectionIds }) }))
}

export async function getCollectionMedia(collectionId: string, signal?: AbortSignal): Promise<MediaFile[]> {
  const response = await fetch(`/api/collections/${encodeURIComponent(collectionId)}/media`, signal === undefined ? undefined : { signal })
