...
import { asForwardedProps, cn } from "@/lib/utils"
...
const forwarded = asForwardedProps(useForwardPropsEmits(delegatedProps, emits))

const id = useId()
const { filterState, filterItems, allItems, allGroups } = useCommand()
...
onMounted(() => {
  ...
  allItems.value.set(id, currentElement.value.textContent)
