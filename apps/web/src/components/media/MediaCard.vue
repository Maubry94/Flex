...
import { Check, CircleCheck, Film, Heart, Pencil } from '@lucide/vue'
import { ref } from 'vue'
...
<template>
  <ContextMenu>
    <ContextMenuTrigger as-child>
      <RouterLink :to="{ name: 'video', params: { mediaId: item.id } }" class="group block">
        <Card class="gap-0 overflow-hidden rounded-2xl border-white/8 bg-card/70 py-0 shadow-none transition group-hover:-translate-y-0.5 group-hover:border-white/15">
          <div class="relative grid aspect-video place-items-center overflow-hidden bg-gradient-to-br from-zinc-800 to-zinc-950">
            ...
            <span v-if="item.favorite" class="absolute left-2 top-2 grid size-7 place-items-center rounded-full bg-black/75 text-primary backdrop-blur" aria-label="Favori"><Heart class="size-3.5 fill-current" /></span>
